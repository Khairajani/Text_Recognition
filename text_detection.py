...
def text_detection():
    ...
    image = cv2.imread('./upload/image.jpg')    
    orig = image.copy()
    (H, W) = image.shape[:2]
    
    r_width = W / float(new_width)
    r_height = H / float(new_height)
    ...
    cv2.imwrite('./upload/detection.jpg',orig)
